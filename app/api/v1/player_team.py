...
from schemas.player_team import CurrentStation, CurrentStationResponse, PlayerTeam, PlayerTeamCreate, ScoreAdd, ScoreResponse
from services.player_team import PlayerTeamService

router = APIRouter(tags=["playerteam"],dependencies=[Depends(IsAuthenticated)])
...
async def add_score_to_player_team(id: int, score: ScoreAdd):
    score_add,current_station = await player_team_service.add_score_to_team(id=id, score_to_add=score.model_dump()['score'])
...
@router.post("/api/playerteam/{id}/set_current_station", response_model=CurrentStationResponse)
