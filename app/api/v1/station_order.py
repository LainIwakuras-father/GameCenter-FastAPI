...
async def create_station_order(order_data:StationOrderCreate):
    return await station_order_service.create_station_order(order_data=order_data.model_dump())

@router.put("/api/stationorder/{id}")
async def update_station_order(id: int,order_data:StationOrderUpdate):
    return await station_order_service.update_station_order(id=id,order_data=order_data.model_dump())
